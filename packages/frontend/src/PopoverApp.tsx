...
export function PopoverApp() {
  ...
  async function handleRunNow() {
    ...
    const timeoutId = setTimeout(() => {
      ...
      setResearchError('Research took too long. Try again with fewer topics.');
    }, 5 * 60 * 1000);
  ...
  return (
    <div className="w-80 h-[520px] bg-white dark:bg-gray-900 rounded-lg shadow-2xl overflow-hidden flex flex-col border border-gray-200 dark:border-gray-700">
      ...
      {/* Research Error */}
      {researchError && (
        <div className="px-3 py-2 bg-red-50 dark:bg-red-900/20 border-y border-red-200 dark:border-red-800">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-red-900 dark:text-red-300">Research Failed</p>
              <p className="text-xs text-red-700 dark:text-red-400 mt-0.5 line-clamp-2">{researchError}</p>
            </div>
            <button
              onClick={() => setResearchError(null)}
              className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-200 p-0.5"
            >
