...
export function HomePage() {
  ...
  useEffect(() => {
    console.log('[HomePage] Initial mount - fetching briefings');
    getTodaysBriefings('initial-mount');
  }, [getTodaysBriefings]);

  // Debug: Log every progress state change
  useEffect(() => {
    console.log('[HomePage] Progress state changed:', {
  ...
  useEffect(() => {
    console.log('[HomePage] Checking refresh condition:', {
      currentPhase: progress.currentPhase,
      isRunning: progress.isRunning,
      shouldRefresh: progress.currentPhase === 'complete' && !progress.isRunning
    });
    if (progress.currentPhase === 'complete' && !progress.isRunning) {
      console.log('[HomePage] Triggering briefings refresh on completion');
      getTodaysBriefings('completion-effect');
    }
  }, [progress.currentPhase, progress.isRunning, getTodaysBriefings]);

  // Listen for refresh signal from CLI (when research completes via CLI)
  useEffect(() => {
    const unlisten = listen('briefings:refresh', () => {
      console.log('Received briefings:refresh event from CLI');
      getTodaysBriefings('cli-refresh-event');
    });
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      const [_] = await Promise.all([
        getTodaysBriefings('manual-refresh-button'),
  ...
  const handleRunResearch = async () => {
    ...
    try {
      ...
      // Error: Refresh to show any partial results that were saved before failure
      await getTodaysBriefings('research-error-handler');
    }
  ...
  */
  ...
  const handleDelete = async (briefingId: string) => {
    ...
    try {
      ...
      await getTodaysBriefings('after-delete');
    } catch (err) {
